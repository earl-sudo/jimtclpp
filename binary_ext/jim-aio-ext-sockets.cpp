...
static int JimParseIpAddress(Jim_InterpPtr interp_, const char* hostport, union sockaddr_any* sa, int* salen) {
...

    {
        ...
        if (prj_getaddrinfo(sthost, nullptr, &req, &ai)) { // #NonPortFuncFix #SockFunc
            ...
            memcpy(&sa->sin, ai->ai_addr, ai->ai_addrlen);
            *salen = ai->ai_addrlen;
            IGNOREPOSIXRET prj_freeaddrinfo(ai); // #NonPortFuncFix #SockFunc
        }
...
static Retval aio_cmd_recvfrom(Jim_InterpPtr  interp_, int argc, Jim_ObjConstArray  argv) // #JimCmd #PosixCmd
{
    ...
    if (Jim_GetLong(interp_, argv[0], &len_) != JRET(JIM_OK)) {
        return JRET(JIM_ERR);
    }
...
static Retval aio_cmd_sendto(Jim_InterpPtr interp_, int argc, Jim_ObjConstArray argv) // #JimCmd #PosixCmd
{
    ...
    if (IPV6 && af->addr_family == PF_INET6) {
        if (JimParseIPv6Address(interp_, addr, &sa, &salen) != JRET(JIM_OK)) {
            return JRET(JIM_ERR);
        }
    } else if (JimParseIpAddress(interp_, addr, &sa, &salen) != JRET(JIM_OK)) {
        return JRET(JIM_ERR);
    }
...
static Retval aio_cmd_accept(Jim_InterpPtr interp_, int argc, Jim_ObjConstArray argv) // #JimCmd #PosixCmd
{
    ...
    if (argc > 0) {
        if (JimFormatIpAddress(interp_, argv[0], &sa) != JRET(JIM_OK)) {
            return JRET(JIM_ERR);
        }
    }
...
static Retval aio_cmd_listen(Jim_InterpPtr interp_, int argc, Jim_ObjConstArray argv) // #JimCmd #PosixCmd
{
    ...
    if (Jim_GetLong(interp_, argv[0], &backlog) != JRET(JIM_OK)) {
        return JRET(JIM_ERR);
    }
...
static Retval aio_cmd_sockopt(Jim_InterpPtr interp_, int argc, Jim_ObjConstArray argv) // #JimCmd #PosixCmd
{
    ...
    if (argc == 0) {
        ...
        for (i = 0; i < sizeof(g_sockopts) / sizeof(*g_sockopts); i++) {
            ...
            if (prj_getsockopt(af->fd_, g_sockopts[i].level_, g_sockopts[i].opt, (void*) &value, &len_) == 0) { // #NonPortFuncFix #SockFunc
    ...
    for (i = 0; i < sizeof(g_sockopts) / sizeof(*g_sockopts); i++) {
        if (strcmp(Jim_String(argv[0]), g_sockopts[i].name_) == 0) {
            int on;
            if (g_sockopts[i].tokenType_ == SOCKOPT_BOOL) {
                if (Jim_GetBoolean(interp_, argv[1], &on) != JRET(JIM_OK)) {
                    return JRET(JIM_ERR);
                }
            } else {
                long longval;
                if (Jim_GetLong(interp_, argv[1], &longval) != JRET(JIM_OK)) {
                    return JRET(JIM_ERR);
                }
                on = longval;
            }
            if (prj_setsockopt(af->fd_, g_sockopts[i].level_, g_sockopts[i].opt, (void*) &on, sizeof(on)) < 0) { // #NonPortFuncFix #SockFunc
